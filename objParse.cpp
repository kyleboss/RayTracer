...
void read_line(istream& stream, vector<Triangle> * objects, vector<Coord> * temp_vertices, vector<Vector> * temp_normals) {
  string line;
  ...
  if (strncmp(l, "v ", 2) == 0) {
    ...
    for (int i = 0; i < tokens.size(); i += 4) {
      ...
      if (temp_normals->size() != 0) { //has normals
        ...
        tri = Triangle(a, b, c, vna, vnb, vnc);
        //cout << "GOT HERE!!!!!!!!!!!!!! " << endl;
      }
...
void objParse(char * filename, vector<Triangle> * objects){
  ...
  if (objFile.is_open()) {
    while (objFile.good()) {
      read_line(objFile, objects, &temp_vertices, &temp_normals);
    }
    objFile.close();
  }
  else {
